...
void DataEngine::loadMarketData(const std::string& json) {
    ...
    if (nullptr == hdf5_plugin_) {
        hdf5_plugin_ = plugin::load_plugin_dynamically("./", "./../logs/", imsimple::info,
                                                       imsimple::err, "hdf5reader");
        if (nullptr == hdf5_plugin_)
            return;
        hdf5_plugin_->init("hdf5reader");
    }
    hdf5_plugin_->doCommandAsync(json, [&](int64_t cmdid, const std::string& retmsg) -> int {
