...
std::string gb2312toutf8(const std::string& gb2312str) {
    ...
    struct codecvt : std::codecvt_byname<wchar_t, char, mbstate_t> {
        codecvt(const char* loc) : std::codecvt_byname<wchar_t, char, mbstate_t>(loc) {}
        ~codecvt() {}
    };
...
std::string& replace_whole(std::string& data, const std::string& s, const std::string& t) {
    ...
    while ((n = data.find(s, n)) != std::string::npos) {
        ...
        n += t.size() - 1;  // “-1”是为了退回一个字符，这样\t\t就可以被循环替换为\t0\t
    }
    return data;
}
...
void rtrim(std::string& s, char token) {
    s.erase(std::find_if(s.rbegin(), s.rend(), [token](int ch) { return token != ch; }).base(),
            s.end());
}
...
bool is_string_nullorempty(char* str) { return (str == nullptr || strlen(str) == 0); }
...
#define max_path 40960
    ...
    if (ret == nullptr) {
        spdlog::error("Failed to get realpath of {}, errorno = {}", path, errno);
    }
    out = resolved_path;
#endif
}
